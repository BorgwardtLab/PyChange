...
def create_lib(n):
    class Vector(object):
        lib = cdll.LoadLibrary('./cpp/MC.so')  # class level loading lib
        ...
        def change(self, filename, seq):  # foo in Python calls foo in C++
            ...
            Vector.lib.change(self.vector, c_char_p(filename.encode('utf-8')), arr, c_int(len(seq)))
...
class MaChaMP:
    ...
    def __init__(self, seq, type="Welch-Fisher"):
        gc.collect()
