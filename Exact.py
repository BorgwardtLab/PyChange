...
def Exact(seq,no=-1,test_loc='TT',offset=2,thres=0.05, constraints=[]):
    #Initialization:
    if test_loc == 'KS':
    ...
    if no == -1:
        ...
        p2 = P.Permutation(test_loc(seq,loc2),loc2,seq,test_loc)
        ...
        p3 = P.Permutation(test_loc(seq,loc3),loc3,seq,test_loc)
    ...
    if no == -1:
        for no_cp in range(4,len(indices)+1):
            ...
            if len(best_loc) == no_cp-1:
                return best_loc, P.Permutation(test_loc(seq,best_loc),loc,seq,test_loc)
    else:
